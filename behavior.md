...
- **`LiveStrategyRunner`** — executes the strategy against a real exchange through a **`LiveExecutionPort`** the host bot implements (place/cancel entry limit, replace/cancel stop-loss & take-profit, market close + fill notifications back into the runner). One runner = one symbol+direction.
  ...
  - **24h turnover outside the player**: `feedClosedBar`/`catchUpBar` take an optional 4th `volume24hUsd?: number` — the host-supplied 24h quote volume of the closed bar (the exchange feed is the only one who knows it). `LiveStrategyRunner.getVolume24h()` returns it for the current bar, `null` when the host omits it. `PaperStrategyRunner` instead feeds it into the runtime's own volume series bar by bar (the player loads a whole series upfront), registered under `MockTradingEnvOptions.resolution` (default `"live"`) so both `getVolume24h()` and `getVolume24h(resolution)` resolve exactly as in the player.
